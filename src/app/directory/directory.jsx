...
export default function Directory({ initialLawyers }) {
  ...
  // OVERLAY STATES FOR LAWYER POPUP
    ...
    // DOWNLOAD ALL LAWYERS PDF
    const getAllLawyersList = async()=>{
      try {
      const response = await fetch(`${base_url}/lawyer-list`)
        ...
        {lawyers.map((lawyer, index)=>{
            ...
            return (
                ...
                <div key={index} className="flex flex-row justify-evenly items-center p-4  w-full rounded-md z-9999">
                  <div onClick={() => { setOverlayLawyerName(lawyer.username); setOverlayLawyerEmail(lawyer.email); setOverlayLawyerPhone(lawyer.phone); setOverlayLawyerPhoto(lawyer.photo); setOverlayLawyerDescription(lawyer.description); setOverlayEnrolmentNumber(lawyer.enrolmentNumber); setOverlayLawyerAddress(lawyer.address); setOverlayLawyerPhoto(lawyer.photo); setOverlayFatherName(lawyer.fatherName); setOverlayLawyerMembership(lawyer.membership); setOverlay(true) }}  className="flex flex-row justify-start flex-wrap items-center py-2 w-full sm:w-4/5">
