...
export default function AdminWrapper() {
  ...
  const fetchData = async () => {
      try {
        const [lawyerRes, eventRes, linkRes, AdminRes, committeeRes] = await Promise.all([
          fetch(`${base_url}/lawyers`),
          fetch(`${base_url}/events`),
          fetch(`${base_url}/links`),
          fetch(`${base_url}/admins`),
          fetch(`${base_url}/executive-committee`)
        ]);

        const [lawyerJson, eventJson, linkJson, adminJson, committeeJson] = await Promise.all([
          lawyerRes.json(),
          eventRes.json(),
          linkRes.json(),
          AdminRes.json(),
  ...
  return (
    <Admin
      ...
      initialCommittee={data.committee || []}
